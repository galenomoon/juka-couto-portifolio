...
export default function NavBar() {
  ...
  function mobileScrollTo(ref: React.RefObject<HTMLDivElement>) {
    setIsOpen(false);
  ...
  return (
    <>
            ...
            <>
              ...
              >
                {navbar_options.map(({ name, ref }, index) => (
                  <motion.button
                    onClick={() =>
                      mobileScrollTo(ref as React.RefObject<HTMLDivElement>)
                    }
                    key={index}
                    variants={item}
      ...
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        className="w-full sm:hidden md:flex justify-between py-10  h-[120px] text-typography-300"
      >
